...
file_name = "seg-_overlap_2.0_best_weights_checkpoint__oversample_bol_True_aug_bol_True_lr_bol_True_flip_bol_False_el_1.0_rot_1.0__encode_encode_layer_loss_start_0001_training__val_5_test_1__july19_epoch.hdf5_guass_False_testing-original_val_5_test_1"
slice_num = 74
# pred starts at dim_offset in
pred_num = slice_num + dim_offset
input_folder = "brain_ohsu/data/testing/testing-original"


# get true labels
vol = read_tiff_stack("brain_ohsu/data/testing/testing-original/labels/cube1_labels.tiff")
...
# get true volumes
vol = read_tiff_stack("brain_ohsu/data/testing/testing-original/volumes/cube1.tif")
...
pred_image_array = np.array(
    Image.open(
        f"brain_ohsu/data/testing/{file_name}/seg-slice000{pred_num}.tiff"
    )
)
...
truth_array = np.array(
    Image.open("brain_ohsu/data/testing/testing-original/slices_truth_labels/slice000{slice_num}.tiff")
)
...
# imgray_pred[edge_locs[0], edge_locs[1], :] = (255,105,180)  # red false positive


im = Image.fromarray(imgray_pred.astype(np.uint8))
im.save(
    f"{file_name}_fp_fn.tif"
)
