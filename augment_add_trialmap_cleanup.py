...
def generate_data_set(data_original_path, data_set_path, nb_examples=10):
    ...
    for i in range(nb_examples):
        ...
        volume_chunk, label_chunk = get_random_training(
            volumes[ind], labels[ind], i, nb_examples
        )
        # adding in to test
...
def augment_spatial(
    ...
    if seg is not None:
        if dim == 2:
            seg_result = np.zeros(
                (patch_size[0], patch_size[1], seg.shape[3]), dtype=np.float16
            )
        else:
            seg_result = np.zeros(
                (patch_size[0], patch_size[1], patch_size[2], seg.shape[3]),
                dtype=np.float16,
            )

    if dim == 2:
        data_result = np.zeros(
            (patch_size[0], patch_size[1], data.shape[3]), dtype=np.float16
        )
    else:
        data_result = np.zeros(
            (patch_size[0], patch_size[1], patch_size[2], data.shape[3]),
            dtype=np.float16,
        )
...
class VolumeDataGenerator(Sequence):
    ...
    def augment_gaussian_blur(
        ...
        if not per_channel:
            sigma = (
                get_range_val(sigma_range)
                if (
                    (not different_sigma_per_axis)
                    or (
                        (np.random.uniform() < p_isotropic) and different_sigma_per_axis
                    )
                )
                ...
                if per_channel:
                    sigma = (
                        ...
                        if (
                            (not different_sigma_per_axis)
                            or (
                                (np.random.uniform() < p_isotropic)
                                and different_sigma_per_axis
                            )
                        )
                        else [get_range_val(sigma_range) for _ in data_sample.shape[1:]]
                    )
    ...
    def _set_params(self):
        ...
        self.width_shift = random.uniform(
            -self.width_shift_range, self.width_shift_range
        )
        self.vertical_shift = random.uniform(
    ...
    def _transform_vol(self, vol):
        for i in range(vol.shape[0]):
            ...
            trans_img = self._shift_img(
                trans_img, self.width_shift, self.vertical_shift
            )
    ...
    def flow(self, x, y, batch_size):
        while True:
            ...
            offset = (input_dim - output_dim) // 2
            y_gen = np.zeros(
                (batch_size,) + crop_numpy(offset, offset, offset, y[0]).shape
            )
            ...
            for i in inds[:batch_size]:
                ...
                write_tiff_stack_paper(
                    preprocess_vol[:, :, :, 0].astype(float),
                    data_set_path + "/look/processed-inputn" + str(i) + ".tiff",
                )
                write_tiff_stack_paper(
                    y_copy[:, :, :, 0].astype(float),
                    data_set_path + "/look/orig-labeln" + str(i) + ".tiff",
                )
                write_tiff_stack_paper(
                    np.sum(y_copy[:, :, :, :].astype(float), -1),
                ...
                write_tiff_stack_paper(
                    x2[:, :, :, 0],
                    data_set_path + "/look/augment_spatial-input-n" + str(i) + ".tiff",
                )
                write_tiff_stack_paper(
                    y2[:, :, :, 0].astype(float),
                    data_set_path + "/look/augment_spatial-labeln" + str(i) + ".tiff",
                )
                write_tiff_stack_paper(
                    np.sum(y2[:, :, :, :].astype(float), -1),
                    data_set_path
                    + "/look/augment_spatial-label-all-infon"
                    + str(i)
                    + ".tiff",
                )
                # keep offset zero until after spatial transformation
...
training_generator = datagen.flow(x_training, y_training, batch_size)

for i in range(1):
    x3, y3 = next(training_generator)
    print(np.unique(x3))
    print(np.unique(y3))