...
def loss_inference(input_folder, output_folder, validation_data = False):
    ...
    loss = edge_f1_score(tensor1, tensor2)
    output_dict["edge_f1_score"] = loss.numpy()
...
def write_folder_section(output_folder, file_names, section_seg):
    ...
    for slice_index in range(dim_offset, section_seg.shape[0] - dim_offset):
...
def segment_brain_gauss(
    ...
    agg_gauss_total = np.zeros((len(file_names), first_img.shape[0] - 2*dim_offset, first_img.shape[1] - 2*dim_offset))
    output_total = np.zeros(agg_gauss_total.shape)
    ones_total = np.zeros(agg_gauss_total.shape)
    ...
    while section_index <= len(file_names) - input_dim:  # + dim_offset:
        ...
        # to overlap in the z direction, collect values from each section and the agg gaussina map into a meta table
        (
            orig_seg,
            aggregated_nb_of_predictions,
            aggregated_ones,
        ) = helper_segment_section(
            model, section_vol, gaussian_importance_map, overlap_var
        )

        output_total[
            section_index + dim_offset : section_index + input_dim - dim_offset, :, :
        ] += orig_seg[dim_offset : input_dim - dim_offset, :, :]
        agg_gauss_total[
            section_index + dim_offset : section_index + input_dim - dim_offset, :, :
        ] += aggregated_nb_of_predictions[
            dim_offset : input_dim - dim_offset, :, :
        ]
        ones_total[
            section_index + dim_offset : section_index + input_dim - dim_offset, :, :
        ] += aggregated_ones[dim_offset : input_dim - dim_offset, :, :]
    ...
    (
        ...
        aggregated_nb_of_predictions,
        aggregated_ones,
    ) = helper_segment_section(model, section_vol, gaussian_importance_map, overlap_var)

    output_total[
        end_aligned + dim_offset : end_aligned + input_dim - dim_offset, :, :
    ] += orig_seg[dim_offset : input_dim - dim_offset, :, :]
    agg_gauss_total[
        end_aligned + dim_offset : end_aligned + input_dim - dim_offset, :, :
    ] += aggregated_nb_of_predictions[dim_offset : input_dim - dim_offset, :, :]
    ones_total[
        end_aligned + dim_offset : end_aligned + input_dim - dim_offset, :, :
    ] += aggregated_ones[dim_offset : input_dim - dim_offset, :, :]

    # pd.DataFrame(ones_total[14,:,:]).to_csv(output_folder + 'agg_one_total_'+ str(overlap_var) + '.csv')
...
def helper_segment_section(model, section, gaussian_importance_map, overlap_var):
    ...
    for x in range(0, temp_section.shape[1] - input_dim, int(output_dim / overlap_var)):
        for y in range(
            0, temp_section.shape[2] - input_dim, int(output_dim / overlap_var)
        ):
            coords.append((0, x, y))
    ...
    while i < len(coords):
        ...
        for j in range(len(batch_coords)):
            ...
            # multiple by gaussian_importance map
            output[j] *= gaussian_importance_map
            seg[z : z + output_dim, x : x + output_dim, y : y + output_dim] += output[j]
            # keep track of total sum of gaussian_importance_map to normalize
            ...
            aggregated_nb_of_predictions[
                z : z + output_dim, x : x + output_dim, y : y + output_dim
            ] += gaussian_importance_map
            aggregated_ones[
                z : z + output_dim, x : x + output_dim, y : y + output_dim
            ] += np.ones_like(gaussian_importance_map)
            # print(np.unique(aggregated_ones, return_counts = True))
    seg = seg[:, dim_offset:-dim_offset, dim_offset:-dim_offset]
    aggregated_nb_of_predictions = aggregated_nb_of_predictions[:, dim_offset:-dim_offset, dim_offset:-dim_offset]
    aggregated_ones = aggregated_ones[:, dim_offset:-dim_offset, dim_offset:-dim_offset]
