...
def segment_brain_guass(
    ...
    agg_gauss_total = np.zeros(
        (len(file_names), first_img.shape[0], first_img.shape[1])
    )
    ...
    while section_index <= len(file_names) - input_dim:  # + dim_offset:
        ...
        (
            orig_seg_cropped,
            aggregated_nb_of_predictions_cropped,
            aggregated_ones_cropped,
        ...
        ] += orig_seg_cropped[dim_offset : input_dim - dim_offset, :, :]
        agg_gauss_total[
            section_index + dim_offset : section_index + input_dim - dim_offset, :, :
        ] += aggregated_nb_of_predictions_cropped[
        ...
        ones_total[
            section_index + dim_offset : section_index + input_dim - dim_offset, :, :
        ] += aggregated_ones_cropped[dim_offset : input_dim - dim_offset, :, :]
    ...
    agg_gauss_total[
        end_aligned + dim_offset : end_aligned + input_dim - dim_offset, :, :
    ] += aggregated_nb_of_predictions_cropped[dim_offset : input_dim - dim_offset, :, :]
    ones_total[
        end_aligned + dim_offset : end_aligned + input_dim - dim_offset, :, :
    ] += aggregated_ones_cropped[dim_offset : input_dim - dim_offset, :, :]
    pd.DataFrame(agg_gauss_total[50,:,:]).to_csv(output_folder + 'agg_gauss_total_'+ str(overlap_var) + '.csv')
    pd.DataFrame(output_total[50,:,:]).to_csv(output_folder + 'output_total_'+ str(overlap_var) + '.csv')
    output_total /= agg_gauss_total
    pd.DataFrame(output_total[50,:,:]).to_csv(output_folder + 'output_total2_'+ str(overlap_var) + '.csv')
...
def helper_segment_section(model, section, gaussian_importance_map, overlap_var):
    ...
    aggregated_nb_of_predictions_cropped = aggregated_nb_of_predictions[
        :,
        dim_offset : dim_offset + section.shape[1],
        dim_offset : dim_offset + section.shape[2],
    ]

    aggregated_ones_cropped = aggregated_ones[
        :,
        dim_offset : dim_offset + section.shape[1],
        dim_offset : dim_offset + section.shape[2],
    ]
