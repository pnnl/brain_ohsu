...
def segment_brain(input_folder, output_folder, model):
    ...
    while section_index <= len(file_names) - input_dim + dim_offset:
       ...
       # to overlap in the z direction, collect values from each section and the agg guassina map into a meta table
        section_seg, orig_seg_cropped, gauss_agg_cropped, aggregated_ones_cropped = helper_segment_section(model, section_vol, gaussian_importance_map)
...
def helper_segment_section(model, section, gaussian_importance_map):
    ...
    return cropped_seg,  orig_seg_cropped, aggregated_nb_of_predictions_cropped, aggregated_ones_cropped 
