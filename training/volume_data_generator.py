...
from batchgenerators.augmentations.crop_and_pad_augmentations import (
    center_crop as center_crop_augfrom,
)
...
class VolumeDataGenerator(Sequence):
    ...
    def augment_spatial(
        ...
        if seg is not None:
            if dim == 2:
                seg_result = np.zeros(
                    (patch_size[0], patch_size[1], seg.shape[3]), dtype=np.float32
                )
            else:
                seg_result = np.zeros(
        ...
        if dim == 2:
            data_result = np.zeros(
                (patch_size[0], patch_size[1], data.shape[3]), dtype=np.float32
            )
        else:
            data_result = np.zeros(
    ...
    def flow(self, x, y, batch_size):
        while True:
            ...
            if self.normal == True:
                ...
                y_gen = np.zeros(
                    (batch_size,)
                    + np.copy(crop_numpy_batch(offset, offset, offset, y)).shape[1:]
                )
