...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "generate_validation_set":
         ...
         # create from training, but only part of image
        data_original_path = base_path + f"/data/training/training-original{add_suffix}"
        # change path here to 100 if not double
        data_set_path = base_path + f"/data/validation/validation-set{add_suffix}"

        # Default value is None

        print(data_original_path, data_set_path, num_volumes)
        generate_data_set(data_original_path, data_set_path, training_example = False, num_volumes = num_volumes)
    ...
    # Generate data set by cropping out 64 length cubes from larger chunks for training
        ...
        data_original_path = base_path + f"/data/training/training-original{add_suffix}"
         # change path here to 100 if not double
        data_set_path = base_path + f"/data/training/training-set{add_suffix}"


        print(data_original_path, data_set_path, num_volumes)
        generate_data_set(data_original_path, data_set_path, training_example = True,  num_volumes = num_volumes)
